...
class GUI:
    def set_chat_window(self, connection):
        ...
        self.message_scrolledtext = scrolledtext.ScrolledText(
    ...
    def __init__(self, connection):
        self.set_chat_window(connection)
        # pygame.init()
    ...
    def handle_key(self, event):
        ...
        if len(text) == 1 and text.startswith("/"):
            ...
            if commands:
                ...
                menu.place(
                    x=self.message_entry.winfo_x(),
                    y=self.message_entry.winfo_y() + self.message_entry.winfo_height(),
                )
                menu.bind(
                    "<<ComboboxSelected>>",
                    lambda e: self.handle_command_selection(event, menu),
                )
